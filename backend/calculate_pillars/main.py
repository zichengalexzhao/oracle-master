...
@http
def calculate_pillars(request):
    try:
        req_body = request.get_json()
        birth_datetime = req_body.get("birth_datetime")
        location = req_body.get("location", {"city": "San Francisco", "longitude": -122.4})
        ...
        if four_pillars.get("warning"):
            response["warning"] = four_pillars["warning"]
